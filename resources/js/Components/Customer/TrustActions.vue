...
const props = defineProps({
    ...
    shopAddress: { type: String, default: 'TechMart KE, Luthuli Avenue, Nairobi' },
...
const whatsappMessage = computed(() => {
    if (props.product) {
        ...
        return encodeURIComponent(
            `Hi TechMart KE! I'm interested in the *${props.product.name}*${variant ? ' (' + variant + ')' : ''} listed at KSh ${price}. Is it available? I'd like to know more.`
        );
    }
    return encodeURIComponent("Hi TechMart KE! I'm looking for a phone/laptop. Can you help me find the best deal?");
});

const whatsappUrl = computed(() => `https://wa.me/${props.phoneNumber}?text=${whatsappMessage.value}`);
const callUrl = computed(() => `tel:+${props.phoneNumber}`);
const formatPhone = computed(() => {
    const p = props.phoneNumber;
