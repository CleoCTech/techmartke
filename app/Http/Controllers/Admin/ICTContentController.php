...
class ICTContentController extends Controller
{
    use LayoutTrait;
    use UploadFileTrait;
    
    public $settings = [
        'model' =>  '\\App\\Models\\ICTContent',
        'caption' =>  "ICT Content",
        'xFolder' =>  "Admin/Pages/ICTContent",
        'indexRoute' =>  "/admin/ict-centre-gallery",
        'storageName' =>  "ictFiles",
        'isList' => true,
        'isCreate' => true,
        'isView' => true,
        'isEdit' => true,
    ...
    public function store(REQUEST $request){
        ...
        try{
            ...
            if($request->hasFile('video_path')){
                $fileVideoName = $this->generateFileName($request->file('video_path'));
            }
            $record = [
                'title' => $request->title,              
            ...
            // if($request->hasFile('cover_image')){
            //     $record['video_path'] = $fileVideoName;
            // }
            if($request->status != 3){
            ...
            // if($request->hasFile('video_path')){
            //     $fileData = ['file' => $request->file('video_path'),'fileName' => $fileVideoName, 'storageName' => $this->settings['storageName'].'\\videos','prevFile' => $this->prevRecord != null? $this->prevRecord['video_path']:null];
            //     if(!$this->uploadFile($fileData)){
            //         $this->isCommit = false;
            //     }
            // }
